...
link to data.world: https://media.data.world/gAxMGZvQQCKGEC4H4YEq_regfit.png

link to data.world: https://media.data.world/DLhcmUmsQoqyu1byieIK_cp_var.png

Linear model selection was difficult for this data set because of the high number of categorical variables. The variables that were numeric were discrete, which does not make for favorable linear models. Nontheless, we tried forward stepwise regression and best subset regression to predict overall survival time. In order for this to work, we had to make a dataframe which only contained numerical variables. In our situation, since the number of variables was manageable, there was no obvious reason to favor forward stepwise regression, but the results for stepwise and best subset were identical. The optimized model is made up of four predictors.

```{r}
regfit.fwd=regsubsets(os_time~.,data=newdata,nvmax=13,method="forward")
summary(regfit.fwd)
renderPlot(plot(regfit.fwd,scale="Cp"))
which.min(summary(regfit.fwd)$cp)

regfit.full=regsubsets(os_time~.,data=newdata)
summary(regfit.full)

regfit.full=regsubsets(os_time~.,data=newdata, nvmax=13)
...
A linear model containing four predictors gives the minimum cp, those being days_to_date_of_last_contact, cn_clusters, integrated_clusters_with_pam50, and integrated_clusters_unsup_exp. As seen in the graphs, the results for both selection methods were the same.
